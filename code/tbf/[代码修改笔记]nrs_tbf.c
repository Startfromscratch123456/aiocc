...

{
	...
	if (cli->tc_in_heap)//该操作最红会调用tbf_cli_compare，而tbf_cli_compare中比较的时候参数正是和上面的相关
		   ...
		   struct nrs_tbf_client *cli)
{
    ...
    /**
     * 在nrs_tbf_head所有rule中,根据每个rule的jobid集合找到和cli中jobid一样的
     * rule即为所求
     */
...
/**
 * 从req的rq_reqmsg字段通过jobid = lustre_msg_get_jobid(req->rq_reqmsg);获得
 * jobid，并赋值给nrs_tbf_client 对象cli，cli通过匹配nrs_tbf_head中rule链表，
 * 找到一个jobid集合存在该jobid的rule，将cli挂在该rule中的cli链表,当然也有一些
 * 引用计数相关的操作
 */
static void
		 ...
		 struct ptlrpc_request *req)
{
	...
	head->th_ops->o_cli_init(cli, req);//.o_cli_init = nrs_tbf_nid_cli_init,类似的
 ...
 * \retval 1 e1 < e2
 */
 //根据代码分析：e1<e2 则e1更靠近根节点
 //结合本函数代码deadline=cli1->tc_check_time + cli1->tc_nsecs 越小 越靠近根节点
 // 因为越小就说明越是先截止
 ...
 */
 /**
  * 函数给出policy得到ptlrpc_nrs_request,需要根据parent确定相应client对应的
  * ptlrpc_nrs_resource：top-level的可以直接获得,其它的可能还需要根据一定的
  * 关系和数据结构查找
  */
			   ...
			   bool moving_req)
{
	...
	if (parent == NULL) {
    /**
     * 说明是请求最高级别的ptlrpc_nrs_resource,直接
     * &((struct nrs_tbf_head *)policy->pol_private)
     * 得到nrs_head,在取其中的th_res（ptlrpc_nrs_resource）
     */
		*resp = &((struct nrs_tbf_head *)policy->pol_private)->th_res;
		return 0;
	}
    /**
     * 如果不是顶级的ptlrpc_nrs_resource,而是第二层的ptlrpc_nrs_resource
     * 根据parent拿到head;根据nrq拿到ptlrpc_request,根据ptlrpc_request中的jobid就可以知道
     * 是哪一个client的请求，找出对应的client.因为 "rule可以添加、移除。列表中的rule改变后，
     * queue必须更新相关的rule",所以要确认client的rule是否变化了,如果变化了要做出相应的改变
     * 更新.
     */
	head = container_of(parent, struct nrs_tbf_head, th_res);
	req = container_of(nrq, struct ptlrpc_request, rq_nrq);
    //最终调用形式 cfs_hash_lookup(head->th_cli_hash, &req->rq_peer.nid);
	//key是什么取决于rule规则的方式
	...
	if (cli != NULL) {
		...
		if (cli->tc_rule_sequence !=
			...
			rule = nrs_tbf_rule_match(head, cli);//nrs_tbf_client中有 tc_jobid成员,head中维持有rule链表
			if (rule != cli->tc_rule)
				nrs_tbf_cli_reset(head, rule, cli);
			else
				nrs_tbf_rule_put(rule);
    ...
    //如果client为空,说明还没有对应的client,client是首次访问
    ...
    //防止在分配的过程中,其它的地方抢在之前生成了相应的client
    //如果发生了这样的情况,就以找到的为主
					   ...
					   bool peek, bool force)
{
	...
	LASSERT(cli->tc_in_heap);
	if (peek) {//只检测request而不处理,因此policy并不从policy中移除
...
/**
        |<---                        cli->tc_nsecs            --->|
        |<---   passed   --->|                                    |
cli->tc_check_time          now                                deadline
 */                  
		deadline = cli->tc_check_time +
			  cli->tc_nsecs;
		LASSERT(now >= cli->tc_check_time);
		passed = now - cli->tc_check_time;
        //可见token是一秒产生tc_rpc_rate个
		ntoken = (passed * cli->tc_rpc_rate) / NSEC_PER_SEC;//1000000000L
		ntoken += cli->tc_ntoken;
		if (ntoken > cli->tc_depth)//tokens是否超过限制
			ntoken = cli->tc_depth;
		if (ntoken > 0) {
			struct ptlrpc_request *req;
			nrq = list_entry(cli->tc_list.next,
			...
			cli->tc_check_time = now;//更新检查点
			list_del_init(&nrq->nr_u.tbf.tr_list);
			if (list_empty(&cli->tc_list)) {//该nrs_tbf_client的请求处理完了
				cfs_binheap_remove(head->th_binheap,//从堆中移除代表该nrs_tbf_client的堆节点
						   &cli->tc_node);
				cli->tc_in_heap = false;//标记nrs_tbf_client中的tc_in_heap为false：表示该nrs_tbf_client不在堆中了 
			} else {
				//该nrs_tbf_client还有请求,但是需要重新对堆进行排列,这是因为在堆中的优先级和tc_check_time以及deadline都有关系
				cfs_binheap_relocate(head->th_binheap,
						     &cli->tc_node);
			}
			CDEBUG(D_RPCTRACE,
            ...
            //在deadline的时候再启动
			hrtimer_start(&head->th_timer, time, HRTIMER_MODE_ABS);//HRTIMER_MODE_ABS绝对时间
		}
	}
			   ...
			   struct ptlrpc_nrs_request *nrq)
{
    ...
    //nrs_request_resource:return nrq->nr_res_ptrs[nrq->nr_res_idx];
	...
	head = container_of(nrs_request_resource(nrq)->res_parent,
     ...
     /**
      * nrs_tbf_req_get中如果cli->tc_list为空
      * cli会从相应堆中移除
      */
	if (list_empty(&cli->tc_list)) {
		...
		if (rc == 0) {
            ...
            //将nrq->nr_u.tbf.tr_list添加到cli->tc_list中
			...
			if (policy->pol_nrs->nrs_throttling) {
				__u64 deadline = cli->tc_check_time +
						 cli->tc_nsecs;      
    /**
     * 新添加ptlrpc_nrs_request对象的deadline更早一些,因此需要取消旧的计时器,重新启动新的计时器
     *
     * rtimer_try_to_cancel(struct hrtimer *timer)，这两个函数的区别是，后者提供了额外的返回值-1，
     * 如果定时器当前正在执行因而无法停止，则返回-1.在这种情况下，hrtimer_cancel会一直等处理程序
     * 执行完毕。另外，如果定时器处于未激活状态，两个函数的返回值都是0，如果处于激活状态
     *（即状态为HRTIMER_STATE_INACTIVE或者HRTIMER_STATE_ENQUEUED），二者都返回1
	*/
