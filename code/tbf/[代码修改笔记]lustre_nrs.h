 ...
 *
 * 一个PTLRPC service至少有一个NRS head处理normal priority RPCs,也可以有第二个
 *  NRS head instance处理high-priority RPCs(但这不是强制要求的).每一个NRS heads
 * 维持着一个有效的policy list,其中只有一个作为fallback policy,并可以选择另一个
 * policy作为primary policy(不是强制要求).
 * 所有通过当前NRS head instance处理的RPC,NRS core首先会选择使用primary policy
 * 来入队RPC(如果primary policy存在的话)
...
struct ptlrpc_nrs_policy {
	...
	/**
	 * Private policy data; varies by policy type -->nrs_tbf_head/net
	 */
	void			       *pol_private;
	/**
...
/**
 * Resources 会被嵌入到两种类型NRS entities:
 * 1. 在NRS policies内部，在policy的私有数据域 ptlrpc_nrs_policy::pol_private
 * 2. 在各种策略作为prime-level scheduling entities的对象中
 * 例如在基于client NID轮询或者类似顺序的策略中，每个唯一的client NID会有一个NRS source
 * 在基于后备文件系统对象的轮询中调度中，每个后备文件系统系统partaking对应着一个Resource
 * NRS resources 之间维持着一个parent-child关系，嵌入到policy instance中的是parent entity，
 * 组成了一个简单的资源层级关系.以后还可以有更复杂的关系
 */
