...
{
    //定义的时候op_policy_init是可选的,因此此操作也可能是空操作
 ...
 * 将NRS head的primary policy过度到tlrpc_nrs_pol_state::NRS_POL_STATE_STOPPING
 * 此时,如果policy没有其它的使用者,就将状态进一步过渡到ptlrpc_nrs_pol_state::NRS_POL_STATE_STOPPED.
 */
 ...
 */
 /**
  * 根据lprocfs的命令启动一个policy,并在ptlrpc_nrs_pol_state的范围内转变状态
  * 如果一个primary policy和当前指定的不同,函数会将新的policy从ptlrpc_nrs_pol_state::NRS_POL_STATE_STARTING 
  * 转变为ptlrpc_nrs_pol_state::NRS_POL_STATE_STARTED,旧的primary policy此时,
  * 如果policy没有其它的使用者,就将状态进一步过渡到ptlrpc_nrs_pol_state::NRS_POL_STATE_STOPPED.
  *
  * 如果fallback policy被指定,表示这是一条 停止当前primary policy的指令,(并且不需要指定其它的的替代policy)
  * 此时,如果policy没有其它的使用者,就将状态进一步过渡到ptlrpc_nrs_pol_state::NRS_POL_STATE_STOPPED.
  * 这种情况下fallback policy 是NRS head中仅存的一个处于active的策略
  */
static int nrs_policy_start_locked(struct ptlrpc_nrs_policy *policy, char *arg)
{
	...
	if (policy->pol_flags & PTLRPC_NRS_FL_FALLBACK) {
		...
		if (policy->pol_state == NRS_POL_STATE_STARTED) {//对于已经启动的policy,检查一下参数
        //参数改变了才需要重新启动,没改变就不需要重新启动
			/**
					      ...
					      bool moving_req)
{
	...
	while (1) {//这里如果是tbf最多循环两次,因为nrs_tbf_res_get一旦在第二级资源成功马上就会返回1
		rc = policy->pol_desc->pd_ops->op_res_get(policy, nrq, res,
							  &tmp, moving_req);
		if (rc < 0) {
			if (res != NULL)
				nrs_resource_put(res);
            //rc<0是因为对应cli没有申请成功,res也就没用yinyong,因此需要释放一个引用计数
			return NULL;
		}
				  ...
				  bool moving_req)
{
	...
	memset(resp, 0, sizeof(resp[0]) * NRS_RES_MAX);//NRS_RES_FALLBACK,NRS_RES_PRIMARY,NRS_RES_MAX
...
static void nrs_resource_put_safe(struct ptlrpc_nrs_resource **resp)
{
	...
	int			  i;
  //NRS_RES_FALLBACK,NRS_RES_PRIMARY,NRS_RES_MAX分别为0,1,2
  //resp [0]也即resp[NRS_RES_FALLBACK], resp[1]也即resp[ NRS_RES_PRIMARY],
	for (i = 0; i < NRS_RES_MAX; i++) {//推测NRS_RES_MAX为2,不确定
		if (resp[i] != NULL) {
...
static inline void nrs_request_enqueue(struct ptlrpc_nrs_request *nrq)
{
	 ...
	 */
    //NRS_RES_FALLBACK,NRS_RES_PRIMARY,NRS_RES_MAX分别为0,1,2
    //nr_res_ptrs [0]也即nr_res_ptrs[NRS_RES_FALLBACK], nr_res_ptrs[1]也即nr_res_ptrs[ NRS_RES_PRIMARY],
...
void ptlrpc_nrs_req_hp_move(struct ptlrpc_request *req)
{
	 ...
	 */
	 //nrs_svcpt2nrs(svcpt, true)返回svcpt对应的high-priority nrs_head
