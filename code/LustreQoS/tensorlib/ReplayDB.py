...
import sqlite3
from typing import *
from .aiocc_logging import logger
...
class ReplayDB:
    ...
    def connect_db(self, opt):
        ...
        if 'disable_same_thread_check' in opt and opt['disable_same_thread_check']:
            #一个线程只能使用当前线程打开的数据库连接，除非在连接时设置了check_same_thread=False参数
    ...
    def get_last_ts(self) -> int:
        ...
        while True:
            ...
            #某个时间戳对应的performance indicator是否等于monitor agent的数量,判断性能指标数据是否已经到齐
    ...
    def get_observation(self, ts: int) -> np.ndarray:
        ...
        for row in data:
            ...
            if ma_id not in self.ordered_client_list:
                ...
                continue
            # ma_id不一定连续,也不一定从0开始,因此这相当于是向数组下标映射的一个过程
            ma_id_idx = self.ordered_client_list.index(ma_id)
            # row[1] - (ts - self.ticks_per_observation + 1)
