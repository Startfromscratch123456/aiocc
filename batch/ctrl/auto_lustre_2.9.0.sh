...
# POSIX
#
#description:    compile-->install-->deploy lustre 2.9.0 automaticlly
#     author:    ShijunDeng
...
print_message "MULTEXU_INFO" "Now start to compile-->install-->deploy lustre 2.9.0 ..."
#基准目录
base_dir=${MULTEXU_BASE_DIR}
#kvm节点备份目录 备份节点配置：IP,root权限密码,免密码登陆认证,常用的个性化配置(屏幕、电源),
kvm_source_bakpath=/home/ca21/DevelopmentFiles/vm/back
#kvm资源文件目录
kvm_source_path=/home/ca21/DevelopmentFiles/vm
#lustre文件系统节点机器的名称
kvm_nodes_name_lustre=("node11" "node12" "node13" "node14" "node15" "node16" "node17")
#lustre文件系统节点机器的资源名称
kvm_source_name_lustre=("node11.qcow2" "node12.qcow2" "node13.qcow2" "node14.qcow2" "node15.qcow2" "node16.qcow2" "node17.qcow2")
#进行编译工作机器节点的名称
kvm_node_name_dev="centos7.0-dev"
kvm_source_name_dev="centos7-dev.qcow2"
#编译节点
...
sh ${MULTEXU_BATCH_CRTL_DIR}/multexu.sh --iptable=${compile_node_ip} --cmd="rm -rf ${base_dir}/AIOCC"
sh ${MULTEXU_BATCH_CRTL_DIR}/multexu.sh --iptable=${compile_node_ip} --cmd="rm -rf /root/kernel/rpmbuild/BUILD/lustre-2.9.0/*"
...
sh ${MULTEXU_BATCH_CRTL_DIR}/multexu.sh --iptable=${compile_node_ip} --cmd="yes | cp /root/kernel/rpmbuild/BUILD/lustre-2.9.0/*.rpm ${MULTEXU_SOURCE_INSTALL_DIR}/ "
`${PAUSE_CMD}`
`${PAUSE_CMD}`

#编译lustre client
sh ${MULTEXU_BATCH_CRTL_DIR}/multexu.sh --iptable=${compile_node_ip} --cmd="sh ${MULTEXU_BATCH_CRTL_DIR}/multexu_ssh.sh  --clear_execute_statu_signal"
sh ${MULTEXU_BATCH_CRTL_DIR}/multexu.sh --iptable=${compile_node_ip} --cmd="sh ${MULTEXU_BATCH_BUILD_DIR}/build_capes_lustre_client.sh --skip_install_dependency=1"
#等待server编译完成
ssh_check_singlenode_status ${compile_node_ip} "${MULTEXU_STATUS_EXECUTE}" ${sleeptime} ${limit}
...
sh ${MULTEXU_BATCH_INSTALL_DIR}/auto_lustre2.9.0_install.sh --skip_install_kernel=${skip_build_kernel}
#等待安装完成
local_check_status "${MULTEXU_STATUS_EXECUTE}"  "${sleeptime}" "${limit}"
print_message "MULTEXU_INFO" "the current node finished to execute auto_lustre2.9.0_install.sh..."
...
#部署文件系统
sh ${MULTEXU_BATCH_CRTL_DIR}/multexu_ssh.sh  --clear_execute_statu_signal
sh ${MULTEXU_BATCH_DEPLOY_DIR}/auto_lustre2.9.0_deploy.sh --mdsnode=${mdsnode} --devname=${devname} --devindex=${devindex}
#等待部署完成
while [[ $(cat ${EXECUTE_STATUS_SIGNAL}) != "${MULTEXU_STATUS_EXECUTE}" ]];
do
        print_message "MULTEXU_INFO" "the current node is executing auto_lustre2.9.0_deploy.sh..."
        sleep ${sleeptime}s
done
print_message "MULTEXU_INFO" "the current node finished to execute auto_lustre2.9.0_deploy.sh..."
...
time_cost=0
time_cost=`echo "scale=6;($end_time_ms - $start_time_ms)/1000000" | bc` 
print_message "MULTEXU_INFO" "Process compile-->install-->deploy lustre 2.9.0 finished..."
print_message "MULTEXU_INFO" "Total time spent:${time_cost} s"
